...
outputs of the rates functions. Such as the below dataframe output. 
...
- For example, `fmt_ci_df(dat)` will produce `r fmt_ci_df(dat)`. All of these suffixes 
  will have three options `e`, `l`, and `u`. These refer to `estimate`, `lower`, 
  and `upper` column positions or names.
...
- `fmt_count()` will count a condition in a data frame and present the number 
  ...
  of women patients from hospital from the below fake dataset. 
```{r counts}
...
You would write:`fmt_count(dat, gender == "f", location == "hospital")` and it 
would produce: `r fmt_count(dat, gender == "f", location == "hospital")` 
